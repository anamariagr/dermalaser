	...
	function importer_page() {
		...
		if ( ! empty( $themes ) || IDEAPARK_DEMO ) {
			...
			if ( ( $code = ideapark_get_purchase_code() ) && $code !== IDEAPARK_SKIP_REGISTER ) {
				...
				$output .= '<p class="submit">' . ( PHP_OS == "Darwin" || ! IDEAPARK_DEMO ? '<button class="button button-primary" ' . ( ! empty( $this->importer['steps'] ) ? ' style="display:none" ' : '' ) . ' id="ip-import-submit">' . __( 'Import', 'ideapark-luchiana' ) . '</button> ' : '' ) . '</p>';
				$output .= '<div class="ip-loading-progress' . ( ! empty( $this->importer['steps'] ) ? ' paused' : '' ) . '"><div class="ip-loading-bar"><div class="ip-loading-state"></div><div class="ip-loading-info">' . __( 'Progress', 'ideapark-luchiana' ) . ': 0%...</div></div><div class="ip-import-output">' . __( 'Prepare data...', 'ideapark-luchiana' ) . '</div></div>';
				if ( ! empty( $this->importer['steps'] ) ) {
					$percent = round( ( $this->importer['base']->step_done / $this->importer['base']->step_total ) * 100 );
					$output  .= '<div class="ip-import-continue">' . ( PHP_OS == "Darwin" || ! IDEAPARK_DEMO ? '<button type="button" class="button button-primary" id="ip-import-continue" data-percent="' . esc_attr( $percent ) . '">' . __( 'Continue', 'ideapark-luchiana' ) . '</button>&nbsp;&nbsp;&nbsp;<button type="button" class="button button-default" id="ip-import-cancel" data-percent="' . esc_attr( $percent ) . '">' . __( 'Cancel', 'ideapark-luchiana' ) . '</button> ' : '' ) . '</div>';
				}
