				...
				while ( $wp_query->have_posts() ) {
					...
					<a class="c-header-search__link" href="<?php the_permalink(); ?>">
						<div class="c-header-search__row">
							<div class="c-header-search__thumb c-header-search__thumb--<?php echo ideapark_mod( 'grid_image_fit' ); ?>">
								<?php the_post_thumbnail( ideapark_mod( 'grid_image_fit' ) == 'cover' ? 'woocommerce_gallery_thumbnail' : 'medium' ); ?>
							</div>
...
if ( ! function_exists( 'ideapark_body_class' ) ) {
	function ideapark_body_class( $classes ) {
		...
		$classes[] = 'h-' . ideapark_mod( 'header_type' );
		...
		if ( ideapark_mod( 'bottom_buttons_mobile_locations' ) == 'screen' && ideapark_mobile_bottom_buttons_cnt() ) {
			$classes[] = 'h-sticky-bottom';
		}
		if ( ideapark_mod( 'bottom_buttons_mobile_locations' ) == 'menu' && ideapark_mobile_bottom_buttons_cnt() ) {
			$classes[] = 'h-menu-bottom';
		}
...
<form role="search" class="js-search-form" method="get" action="' . esc_url( home_url( '/' ) ) . '">
	<div class="c-header__search-form">
		...
		<button class="js-search-clear h-cb c-header__search-clear' . ( ! ideapark_mod( 'ajax_search' ) ? ' no-ajax' : '' ) . '" type="button"  aria-label="' . esc_attr__( 'Clear', 'luchiana' ) . '"><i class="ip-close-small c-header__search-clear-svg"></i><span class="c-header__search-clear-text">' . esc_html__( 'Clear', 'luchiana' ) . '</span></button>
		<button type="submit" class="c-header__search-submit h-cb h-cb--svg" aria-label="' . esc_attr__( 'Search', 'luchiana' ) . '"><i class="' . ( ideapark_mod( 'custom_header_icon_search' ) ?: 'ip-z-search-bold' ) . '"></i></button>
	</div>' . $lang . '
	' . ( ideapark_mod( 'ajax_search_post_type' ) == 'products' ? '<input type="hidden" name="post_type" value="product" class="js-ajax-search-type" />' : '' ) . '
</form>';
...
if ( ! function_exists( 'ideapark_get_account_link' ) ) {
	function ideapark_get_account_link( $prefix = '' ) {
		...
		return ideapark_wrap( $link_title, '<a class="c-header__button-link c-header__button-link--account" title="' . esc_attr( is_user_logged_in() ? esc_html__( 'Account', 'luchiana' ) : esc_html__( 'Login', 'luchiana' ) ) . '" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '" rel="nofollow" aria-label="' . esc_attr__( 'My Account', 'luchiana' ) . '">', '</a>' );
	}
}
...
if ( ! function_exists( 'ideapark_custom_meta_boxes' ) ) {
	function ideapark_custom_meta_boxes( $meta_boxes ) {
		...
		$fields = [
			[
				'type' => 'custom_html',
				'std'  => sprintf( __( 'If nothing is selected, the %s default settings %s will be used', 'luchiana' ), '<a href="' . admin_url( 'customize.php?autofocus[control]=footer_page' ) . '">', '</a>' ),
			],
			[
				'name'    => __( 'HTML block', 'luchiana' ),
				'id'      => 'footer_page',
				'type'    => 'select',
				'std'     => 'default',
				'options' => $options,
	...
	function ideapark_breadcrumb_list() {
		...
		if ( is_front_page() ) {
			...
			$array_list[] = [
				...
				'title'    => __( 'Tag', 'luchiana' ) . ': ' . $current_tag_name,
				'location' => 'is_tag',
			];
		} else if ( is_author() ) {
...
if ( ! function_exists( 'ideapark_header_params' ) ) {
	function ideapark_header_params() {
		...
		if ( $result === null ) {
			...
			$result = [
				'header_height'       => ( is_single() || is_home() || is_archive() || ideapark_is_shop() ) ? '' : 'low',
				'image_id'            => '',
				'bg_color'            => '',
				'header_text_color'   => '',
				'header_menu_color'   => '',
			...
			if (
				...
				( ideapark_is_shop() && ( $post_id = wc_get_page_id( 'shop' ) ) ) ||
...
if ( ! function_exists( 'ideapark_social_networks' ) ) {
	function ideapark_social_networks( $filter = [] ) {
		$networks = [
			...
			'line'      => __( 'Line', 'luchiana' ),
			'wechat'    => __( 'WeChat', 'luchiana' ),
			'viber'     => __( 'Viber', 'luchiana' ),
			'discord'   => __( 'Discord', 'luchiana' ),
			'twitter'   => __( 'X (Twitter)', 'luchiana' ),
...
if ( ! function_exists( 'ideapark_mobile_bottom_buttons_cnt' ) ) {
	function ideapark_mobile_bottom_buttons_cnt() {
		static $mobile_bottom_buttons_cnt;
		if ( $mobile_bottom_buttons_cnt === null ) {
			$header_blocks = ideapark_parse_checklist( ideapark_mod( 'bottom_buttons_mobile' ) );
			foreach ( $header_blocks as $block_index => $enabled ) {
				if ( $enabled ) {
					$mobile_bottom_buttons_cnt ++;
				}
			}
		}

		return $mobile_bottom_buttons_cnt;
	}
}
...
if ( IDEAPARK_IS_AJAX_SEARCH ) {
	...
	add_filter( 'elementor/fonts/groups', function ( $font_groups ) {
		if ( get_theme_mod( 'custom_fonts' ) ) {
			$font_groups = array_merge( [ 'ip-custom-fonts' => __( 'Custom', 'luchiana' ) ], $font_groups );
		}
