...
if ( function_exists( 'rank_math_the_breadcrumbs' ) ) {
	rank_math_the_breadcrumbs([
		...
		'wrap_before' => '<nav aria-label="breadcrumbs" class="rank-math-breadcrumb c-breadcrumbs"><div class="c-breadcrumbs__list">',
		'wrap_after'  => '</div></nav>',
	]);
			...
			foreach ( $breadcrumb_items as $item_index => $item ):
				...
				<li class="c-breadcrumbs__item <?php if ( $with_hidden && $is_last ) { ?>h-hidden <?php } ?><?php ideapark_class( ! $item_index, 'c-breadcrumbs__item--first' ); ?> <?php ideapark_class( $item_index == sizeof( $breadcrumb_items ) - 1, 'c-breadcrumbs__item--last' ); ?>"
					...
					<?php if ($item['link']) { ?><a itemprop="item" title="<?php echo esc_attr( $title ); ?>" href="<?php echo esc_url( $link ); ?>"><?php } ?><span
							itemprop="name"><?php echo esc_html( $title ); ?></span><?php if ($item['link']) { ?></a><?php } ?><?php if ( ! $is_last && ! ( $with_hidden && $is_previous ) ) { ?><!--
